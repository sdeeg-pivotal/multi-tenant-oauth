...
public class TodoController {
    ...
    public String todo(Model model) {
        ...
        System.out.println("Looking for todos at: "+resourceServerUrl);
    ...
    @GetMapping("/todos-failover")
    public String todoFailover(Model model) {
        if (resourceServerUrl.equals("https://resource-server-sample.<your-domain>.com")) {
            model.addAttribute("header", "Warning: You need to configure the Resource Server sample application");
            model.addAttribute("displayWarning", true);
            return "configure_warning";
        ...
        try {
            model.addAttribute("todoList", todoService.getAllFailover());
        } catch (WebClientResponseException error) {
            model.addAttribute("error", error);
            model.addAttribute("todoList", new ArrayList<Todo>());
        }
    ...
    @PostMapping("/todos-failover")
    public String createFailover(@ModelAttribute TodoRequest body) {
        todoService.createFailover(body);
        return "redirect:/todos-failover";
    }

    @DeleteMapping("/todos-failover/{id}")
    public String deleteFailover(@PathVariable String id) {
        todoService.deleteFailover(id);
        return "redirect:/todos-failover";
    }
